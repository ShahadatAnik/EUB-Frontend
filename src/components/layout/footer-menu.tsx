...
const FooterMenu = () => {
	...
	const admissionLinks = navLinks.find((link) => link.title === 'Admission')?.children;

	const usefulLinks = navLinks.find((link) => link.title === 'Useful Links')?.children;
			...
			<div className=''>
				...
				<ul className='space-y-2'>
					{aboutLinks?.map((link, index) => (
						<li key={index}>
							<Link className={pathName === link.href ? 'underline' : ''} href={link.href!}>
								{link.title}
							</Link>
						</li>
					))}
					...
					<li>
						<Link className={pathName === '/contact-us' ? 'underline' : ''} href='/contact-us'>
							Contact Us
						</Link>
					</li>
				</ul>
