...
export type SystemTableColumn<T> = {
	...
	cell?: (value: string, row: T, index: number) => string | number | React.ReactNode;
	cellClassName?:
		| string
		| ((row: T, index: number, isFirstRow: boolean, isLastRow: boolean) => string);
	type?: CellType;
	rowSpan?: (row: T, index: number, isFirstRow: boolean, isLastRow: boolean) => number;
	colSpan?: (row: T, index: number, isFirstRow: boolean, isLastRow: boolean) => number;
			...
			<TableBody>
				...
				{!isLoading &&
					...
					data.map((item: any, rowIndex: number) => (
						<TableRow
							key={rowIndex}
							className={rowClassName ? rowClassName(item as T, rowIndex) : undefined}
						>
							{columns
								.filter((column) => column.isHidden !== true)
								.map((column, colIndex) => {
									// Skip rendering if skipRender function returns true
									if (column.skipRender && column.skipRender(item as T, rowIndex)) {
										return null;
									}
		...
		case 'currency':
			return (
				<NumericFormat
					...
					disabled
					decimalScale={2}
					className={cn('block w-full bg-transparent', value === 0 ? 'text-center' : '')}
