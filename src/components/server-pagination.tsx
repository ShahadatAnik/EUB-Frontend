...
const createPageUrl = (pathName: string, page: number, searchParams: URLSearchParams) => {
			...
			<PaginationContent className='w-full md:max-w-fit'>
				<PaginationItem>
					<Button variant={'ghost'} disabled={current_page === 1}>
						<PaginationPrevious href={createPageUrl(pathName, current_page - 1, searchParams)} />
					</Button>
				</PaginationItem>
						...
						return Array.from({ length: end - start + 1 }, (_, i) => {
							...
							return (
								<PaginationLink
									...
									className={isSelected ? 'rounded border-2 border-primary shadow-lg' : ''}
				...
				<PaginationItem>
					<Button variant={'ghost'} disabled={total_page === 0 || current_page === total_page}>
						<PaginationNext href={createPageUrl(pathName, current_page + 1, searchParams)} />
					</Button>
				</PaginationItem>
			</PaginationContent>
		</Pagination>
