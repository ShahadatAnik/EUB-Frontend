...
const Content: React.FC<{ data: IClubAndSociety[] }> = ({ data }) => {
	...
	const content = data.find((faculty) => faculty.value === currentFaculty)?.content || '';

	const clubs = data.find((faculty) => faculty.value === currentFaculty)?.clubs || [];
				...
				<Accordion type='single' collapsible className='w-full'>
					{clubs.map((club, index) => (
						<AccordionItem value={club.title} key={index}>
							...
							<AccordionContent className='p-8'>{club.content}</AccordionContent>
						</AccordionItem>
					))}
				</Accordion>
			)}
