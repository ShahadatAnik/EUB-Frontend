	...
	return (
		<ContentWrapper title='Office of the IQAC' className='space-y-4'>
			<div className='flex overflow-hidden rounded-md border'>
				<div className=''>
					<Image src={'/person-placeholder.jpg'} alt={'Placeholder'} height={160} width={160} />
				</div>
			...
			<div className='grid grid-cols-2 gap-4'>
				{data.map((faculty, index) => (
					<div key={index} className='flex overflow-hidden rounded-md border'>
						<div className=''>
							<Image src={faculty.image} alt={faculty.name} height={160} width={160} />
						</div>

						<div className='flex flex-col gap-0 px-6 py-3'>
							<div>
								<h3 className='text-xl font-semibold'>{faculty.name}</h3>
								<p className='mt-1 text-muted-foreground'>{faculty.designation}</p>
