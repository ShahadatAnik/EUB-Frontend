...
const PageLayout: React.FC<{
	...
	const pathInitial = '/' + pathName.split('/')[1] + '/' + pathName.split('/')[2] || '';
				...
				<StickySidebar className='lg:w-full' containerClassName='lg:pt-0'>
					...
					>
						{pages.map((item, index) => {
							...
							return (
								<AccordionItem key={index} value={slugify(item.title, { lower: true })}>
									{item.href ? (
										<Link
											target={item.redirect ? '_blank' : '_self'}
											href={item.redirect ? item.href : `${pathInitial}/${item.href}`}
										>
											<AccordionTrigger
												showIcon={!!item.items}
												className={cn(
													'h-12 bg-primary px-4 text-white',
													pathName === pathInitial + item.href && 'font-medium underline'
												)}
											>
												{item.title}
											</AccordionTrigger>
												...
												<AccordionContent className='flex flex-col space-y-1 pb-0 pl-2 pt-1'>
													{item.items.map((child, index) => {
														const itemUrl =
															...
															child.href!;
														return (
															<Link key={index} href={itemUrl} className='h-full w-full'>
																<Button
																	className={cn(
																		'h-full w-full justify-start text-wrap rounded-none bg-gray-700 text-start',
																		pathName === itemUrl && 'font-medium underline'
																	)}
																>
																	{child.title}
																</Button>
