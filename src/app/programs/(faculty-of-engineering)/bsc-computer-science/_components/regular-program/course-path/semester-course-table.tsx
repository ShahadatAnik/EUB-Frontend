		...
		<Table>
			...
			<TableBody>
				{data.map((data, index) => (
					<TableRow key={index}>
						...
						<TableCell className='border-r'>{data.contact_hours_theory}</TableCell>
						<TableCell className='border-r'>{data.contact_hours_sessional}</TableCell>
			...
			<TableFooter>
				<TableRow>
					...
					<TableCell>{data.reduce((total, item) => total + Number(item.credit), 0)}</TableCell>
					<TableCell></TableCell>
				</TableRow>
			</TableFooter>
		</Table>
