			...
			<div>
				...
				<div className='space-y-8'>
					{Object.entries(groupedByYear)
						...
						.map(([year, semesters]) => {
							const yearTotal = semesters.reduce((sum, semester) => sum + semester.total, 0);

							return (
								<div key={year} className='mb-8'>
									<h4 className='mb-4 font-semibold'>
										{getYearOrdinal(Number.parseInt(year))} Year ({yearTotal} Credits)
									</h4>
										...
										<Table>
											...
											<TableBody>
												{semesters.map((semester) => {
													...
													return (
														<React.Fragment key={`${semester.year}-${semester.semester}`}>
															{/* Course rows */}
															{semester.courses.map((course, courseIndex) => (
																<TableRow key={`${course.courseCode}-${course.slNo}`}>
                                          ...
                                          <div className='text-xs text-red-600 mt-1 font-semibold'>
                                            WAIVED
															...
															{/* Semester total row */}
															<TableRow className={cn('bg-secondary font-semibold')}>
																{/* Empty cell for Sl. No. */}
																<TableCell className='border-r'></TableCell>
																{/* Empty cell for Course Code */}
																<TableCell className='border-r'></TableCell>
																{/* Total label in Course Title */}
																<TableCell className='border-r text-right'>Total</TableCell>
												...
												<TableRow className='border-t-2 bg-primary font-bold hover:bg-primary'>
													...
													<TableCell colSpan={3} className='border-r text-right text-white'>
														Total
													</TableCell>
													<TableCell className='text-center text-white'>{yearTotal}</TableCell>
												</TableRow>
											</TableBody>
										</Table>
									</div>
